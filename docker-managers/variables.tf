...
variable "user_data" {
  description = "A User Data script to execute while the server is booting. We recommend passing in a bash script that executes the run-docker script, which should have been installed in the AMI by the install-docker module."
}
